...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const resSum = await axios.get('http://localhost:3000/api/transaksi/summary', {
          headers: { Authorization: `Bearer ${token}` }
        });

        setSummary(resSum.data);
        const resList = await axios.get('http://localhost:3000/api/transaksi', {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const handleSimpan = async (e) => {
      ...
      const token = localStorage.getItem('token');
      try {
        await axios.post('http://localhost:3000/api/transaksi', { ...formData, jumlah: parseInt(formData.jumlah) }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdate = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.put(`http://localhost:3000/api/transaksi/${editData.id}`, {
  ...
  const handleHapus = async (id) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`http://localhost:3000/api/transaksi/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      window.location.reload();
    } catch (err) { console.error(err); }
  };
